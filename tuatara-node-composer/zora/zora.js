...
function Zora() {
    ...
    function zoraQuery() {
        ...
        let graphql = JSON.stringify({
            query: `{
                medias(first:10 orderBy:createdAtTimestamp orderDirection:desc){
                    createdAtTimestamp
                    id
                    contentURI
                      ownerBidShare
        ...
        fetch("https://api.thegraph.com/subgraphs/name/ourzora/zora-v1", requestOptions)
            ...
            .then(result => parseZora(JSON.parse(result)))
            .catch(error => console.log('error', error));
    }

    function parseZora(r) {
        let medias = r.data.medias
        console.log(medias);
        //? parse Medias
        for (let i = 0; i < medias.length; i++) {
            let uri = medias[i].contentURI
            console.log(uri);
        };
    }
    //? Save Local Data
    function writeF(artworksDat) {
        ...
        fs.writeFile('D:/Projects/TuraTara/Repo/TuaTara-stream-bot/tuatara-scene-left/src/zoraData.json', JSON.stringify({ artworksDat }), function (err) {
            if (err) throw err;
            console.log('Saved!');
        });
    }
